...
def findWord():
    ...
    engl_word = ''
    klingon_word = ''
    for i,j in zip(klingon_words, english_words): # klingon_words and english_words are parallel lists
        if i[0] == con[0]:
            engl_word = j
            klingon_word = i
...
def askTranslate(engl_word, klingon_word):
    '''
    This function tests if the user translates the english word to klingon properly
    :param engl_word: str
